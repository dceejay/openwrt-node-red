-- Copyright 2018 Dave Conway-Jones
...
function index()
	...
	entry({"admin", "status", "nrlog"}, call("action_nrlog"), _("Node-RED Log"), 6)

	entry({"admin", "status", "processes"}, form("admin_status/processes"), _("Processes"), 7)

	entry({"admin", "status", "realtime"}, alias("admin", "status", "realtime", "load"), _("Realtime Graphs"), 8)
...
function action_nrlog()
	local nrlog = luci.sys.exec("cat /var/log/node-red.log")
	luci.template.render("admin_status/nrlog", {nrlog=nrlog})
end
